...
const ProductCard = ({ product, onAddToCart, onQuickView, onToggleFavorite }: ProductCardProps) => {
  const getStockStatus = () => {
    if (product.stock === 0) return { status: 'out', label: 'Out of Stock', className: 'stock-out' };
    if (product.stock <= 5) return { status: 'low', label: `${product.stock} left`, className: 'stock-low' };
    ...
    <Card className="product-card group overflow-hidden border-0 shadow-md hover:shadow-lg bg-card">
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          ...
          {isOnSale && (
            <Badge variant="destructive" className="bg-price-sale text-primary-foreground">
              Sale
            </Badge>
          )}
        </div>
